...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1., num_patches=0,base_keep_rate=1., depth=0,save_tokens=0):
        ...
        self.rank_based_attn_bias = nn.Parameter(
            torch.zeros(1, self.num_heads, self.num_patches + self.save_tokens * 4, self.num_patches + self.save_tokens * 4))  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x, keep_rate=None, tokens=None, alive_idx=None):
        ...
        if self.depth > 3 and self.depth <= 9:
            leading_tokens = math.ceil(self.base_keep_rate * (N - ST))
            attn[:,:,ST:2 * ST,2 * ST:leading_tokens + ST] = 0

        if self.depth in [3,6,9]:
            attn_extra = attn[:,:,0:ST,:].clone()
            leading_tokens = math.ceil(self.base_keep_rate * (N - ST))
            lagging_tokens = (N - ST) - leading_tokens
            attn_extra[:,:,:,N - lagging_tokens:] = 0
            attn_extra = attn_extra.softmax(dim=-1)
            attn_extra = self.attn_drop(attn_extra)
            token_extra = (attn_extra @ v).transpose(1, 2).reshape(B, ST, C)
            reconstruction_token = [x[:,:ST].mean(1), token_extra[:,:ST].mean(1)]
...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, alive_idx=None, reconstruction_token_list=[]):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ST = self.save_tokens
            left_tokens = math.ceil(keep_rate * (N - ST))
            if tokens is not None:
                left_tokens = tokens

            cls_attn = attn[:, :ST, ST:].mean(1)  # [B, N, N] -> [B, N-ST-1]
            _, idx = torch.topk(cls_attn, left_tokens, dim=1, largest=True, sorted=True)  # [B, ratio * (N-ST-1)]

            x_img = x[:, ST:] # [B, N-ST-1, C]
            x_dense = batch_index_select(x_img, idx) # [B, N-ST-1, C] -> # [B, ratio * (N-ST-1), C]

            # alive_idx = torch.cat((alive_idx[:, 0:ST+1],torch.gather(alive_idx[:,ST+1:], dim=1, index=idx)),dim=1)

            if self.fuse_token and idx.shape[1] != x_img.shape[1]:
                compl = complement_idx(idx, N - ST)  # [B, (1-ratio) * (N-ST-1)]
                ...
                if self.depth in [3,6]:
                    x = torch.cat([x[:, 0:ST],x[:, 0:ST], x_dense, extra_token], dim=1)
                else:
                    x = torch.cat([x[:, 0:ST], x_dense, extra_token], dim=1)
                ...
                if self.depth in [3,6]:
                    x = torch.cat([x[:, 0:ST],x[:, 0:ST], x_dense], dim=1)
                else:
                    x = torch.cat([x[:, 0:ST], x_dense], dim=1)

        x = x + self.drop_path(self.mlp(self.norm2(x)))
        n_tokens = x.shape[1] - self.save_tokens
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', keep_rate=(1, ), fuse_token=False, base_keep_rate=1., save_tokens=0):
        """
        Args:
        ...
        self.num_tokens = self.save_tokens + 5 if distilled else self.save_tokens + 4
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 4 + self.save_tokens, embed_dim))
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        x = self.neck(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, :self.save_tokens].mean(1)), left_tokens, idxs,reconstruction_token_list
        else:
            return x[:, :self.save_tokens].mean(1), x[:, self.save_tokens * 2], idxs, reconstruction_token_list
...
@register_model
def SvitS_224_r3_6rb_4cls_cp4sto_neck_rploss(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = SvitS_224_r3_6rb_4cls_cp4sto_neck_rploss(base_keep_rate=0.7)
    img_size = 224
