...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, alive_idx=None):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ...
            if self.fuse_token and idx.shape[1] - x_img.shape[1] >= 1:
...
@register_model
def SvitS_224_r3_6rb_4sto_neck_gather(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = SvitS_224_r3_6rb_4sto_neck_gather(base_keep_rate=0.7)
    img_size = 224
