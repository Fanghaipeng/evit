...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, alive_idx=None, reconstruction_token_list=[]):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ...
            x_img = x[:, ST + 1:]
            N_new = x_img.shape[1]
            x_dense = x_img[:, :left_tokens]
            _, idx = torch.topk(cls_attn[:, :left_tokens], left_tokens, dim=1, largest=True, sorted=True) 
            x_dense = batch_index_select(x_dense, idx)

            alive_idx = torch.cat((alive_idx[:, 0:ST+1],torch.gather(alive_idx[:,ST+1:ST+1+left_tokens], dim=1, index=idx)),dim=1)

            if self.fuse_token and idx.shape[1] != x_img.shape[1]:
                x_sparse = x_img[:, left_tokens:]
                sparse_weight = cls_attn[:, left_tokens:]
...
@register_model
def SvitS_224_r3_6rb_4sto_neck_rploss_fd(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = SvitS_224_r3_6rb_4sto_neck_rploss_fd(base_keep_rate=0.7)
    img_size = 224
