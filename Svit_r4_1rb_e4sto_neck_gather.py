...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1., num_patches=0,base_keep_rate=1., depth=0,num_es_dt=0):
        ...
        self.rank_based_attn_bias = nn.Parameter(
            torch.zeros(1, 1, self.num_patches + 1 + self.num_es_dt * 3, self.num_patches + 1 + self.num_es_dt * 3))  # 2*Wh-1 * 2*Ww-1, nH
...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, alive_idx=None):
        ...
        if self.depth in [0]:
            ...
            x_dense = torch.gather(x_img, dim=1, index=idx.unsqueeze(-1).expand(-1, -1, C))
            ...
            if self.fuse_token and idx.shape[1] != x_img.shape[1]:
                ...
                x_sparse = torch.gather(x_img, dim=1, index=compl.unsqueeze(-1).expand(-1, -1, C))  # [B, (1-ratio) * (N-ST-1), C]
                
                # sparse_weight = batch_index_select(cls_attn, compl) # [B, (N-DT-1)] -> # [B, (1-ratio) * (N-DT-1)]
                sparse_weight = torch.gather(cls_attn, dim=1, index=compl)  # [B, N-1-left_tokens] # [B, (N-ST-1)] -> # [B, (1-ratio) * (N-ST-1)]
...
@register_model
def SvitS_224_r4_1rb_e4sto_neck_gather(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = SvitS_224_r4_1rb_e4sto_neck_gather(base_keep_rate=0.7)
    img_size = 224
