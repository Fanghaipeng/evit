...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1., num_patches=0,base_keep_rate=1., depth=0,num_es_dt=0):
        ...
        self.rank_based_attn_bias = nn.Parameter(
            torch.zeros(1, self.num_heads, self.num_patches + 1 + self.num_es_dt * 4, self.num_patches + 1 + self.num_es_dt * 4))  # 2*Wh-1 * 2*Ww-1, nH
...
@register_model
def SvitS_224_r4_6rb_e4sto(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
@register_model
def SvitS_288_r4_6rb_e4sto(pretrained=False, base_keep_rate=0.6, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, base_keep_rate=base_keep_rate, num_es_dt=4)
...
if __name__ == "__main__":
    ...
    model = SvitS_224_r4_6rb_e4sto(base_keep_rate=0.7)
    img_size = 224
