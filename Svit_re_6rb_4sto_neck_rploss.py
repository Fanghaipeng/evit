...
class Attention(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, alive_idx=None):
        ...
        if self.depth >= 2 and self.depth <= 9:
...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, alive_idx=None, reconstruction_token_list=[]):
        ...
        if tokens is not None:
            left_tokens = tokens
        if self.depth <= 3:
            cls_attn = attn[:, :, ST + 1:].mean(dim=1) #[B, N, N-ST-1] -> #[B, N-ST-1]
        else:
        ...
        if self.fuse_token and idx.shape[1] != x_img.shape[1] and self.depth in [3,6,9]:
...
@register_model
def SvitS_224_re_6rb_4sto_neck_rploss(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = SvitS_224_re_6rb_4sto_neck_rploss(base_keep_rate=0.7)
    img_size = 224
