...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1., base_keep_rate=1., depth=12 ,save_tokens=0):
        super().__init__()
    ...
    def forward(self, x, pos, keep_rate=None, tokens=None, alive_idx=None):
        ...
        mask = torch.cat((torch.ones((B, ST+1, N-(ST+1)),device=x.device), mask), dim = 1)
        mask = torch.cat((torch.ones((B, N, ST+1),device=x.device), mask), dim = 2)

        mask = mask.unsqueeze(1)
        
        #NOTE： attn 乘 v 时 加上权重
        self.gamma.data.clamp_(0., 1.)
        attn = (1 - self.gamma) * attn + self.gamma * mask
        #! end
        ...
        #! new for ps2
        attn_mean = torch.mean(attn[:,:,ST+1:,ST+1:], dim=1)
        pos[:,1:] = (attn_mean @ pos[:,1:]).detach()
        #! end
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ...
            if left_tokens == N - (ST + 1):
                return x, pos, None, None, None, left_tokens, alive_idx
            ...
            return x, pos, index, idx, cls_attn, left_tokens, alive_idx
...
class Block(nn.Module):
    ...
    def forward(self, x, pos, keep_rate=None, tokens=None, get_idx=False, alive_idx=None):
        if keep_rate is None:
        ...
        tmp, pos_tmp, index, idx, cls_attn, left_tokens, alive_idx = self.attn(self.norm1(x), pos, keep_rate, tokens, alive_idx)
        pos = (pos + pos_tmp)/2
        # ! new for ps2
        ...
        return x, pos, n_tokens, None, alive_idx
...
class EViT(nn.Module):
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x, pos, left_token, idx, alive_idx = blk(x, pos, keep_rate[i], tokens[i], get_idx, alive_idx)
            # ! end
...
@register_model
def deit_small_patch16_shrink_base_l2nga_lfb_4sto_fix01(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    # model = deit_small_patch16_shrink_base_l2nga_lfb_4sto_fix01(num_classes=1000, base_keep_rate=0.7).cuda()
    # img_size = 224
    # x = torch.randn(16, 3, img_size, img_size).cuda()
    # model(x)
    model = deit_small_patch16_shrink_base_l2nga_lfb_4sto_fix01(num_classes=1000, base_keep_rate=0.5).cuda()
