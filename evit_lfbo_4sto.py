...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1., base_keep_rate=1., depth=0,save_tokens=0):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(1, 1, 197 + self.save_tokens, 197 + self.save_tokens))  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x, keep_rate=None, tokens=None, alive_idx=None):
        ...
        #! relative_position
        relative_position_bias = self.relative_position_bias_table[:,:,:N,:N].expand(B,self.num_heads,-1,-1)
...
@register_model
def deit_small_patch16_shrink_base_lfbo_4sto(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = deit_small_patch16_shrink_base_lfbo_4sto(base_keep_rate=0.7)
    img_size = 224
