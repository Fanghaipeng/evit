...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, save_num=0, attn_drop=0., proj_drop=0., keep_rate=1., base_keep_rate=1.):
        super().__init__()
        ...
        #     torch.zeros(1, num_heads, 197 + 3, 197 + 3))  # 2*Wh-1 * 2*Ww-1, nH

        # trunc_normal_(self.relative_position_bias_table, std=.02)

    def forward_global(self, q, k, v):
        B, num_heads, N, C = q.shape
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        x = (attn @ v).transpose(1, 2).reshape(B, N, C * num_heads)
        return x, attn
    ...
    def forward(self, x, keep_rate=None, tokens=None, alive_idx=None):
        ...
        ST = self.save_num
        B, N, C = x.shape
        IT_counts = math.ceil((1 - self.base_keep_rate) * (N - ST - 1))
        IT_begin = N - IT_counts
        ...
        x_glb = x[:, :ST, :]
        q_img, k_img, v_img = q[:, :, ST:], k[:, :, ST:], v[:, :, ST:]
        q_glb, _, _ = q[:, :, :ST], k[:, :, :ST], v[:, :, :ST]
        # q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)

        # base global attention
        x_img, attn_img = self.forward_global(q_img, k_img, v_img)

        # global aggregation
        x_glb = self.forward_global_aggregation(q_glb, k_img[:,:,IT_begin:], v_img[:,:,IT_begin:])
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 fuse_token=False, depth=12, save_num=0, base_keep_rate=1.):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.save_num = save_num
        self.base_keep_rate = base_keep_rate
        self.attn = Attention(dim, num_heads=num_heads, save_num=self.save_num, qkv_bias=qkv_bias,
                              attn_drop=attn_drop, proj_drop=drop, keep_rate=keep_rate, base_keep_rate=self.base_keep_rate)
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, alive_idx=None):
        ...
        if index is not None:
            ...
            if self.fuse_token and self.depth in [3,6,9]:
                ...
                extra_idx = int(196 + ST + self.depth/3)
                alive_idx = torch.cat((alive_idx,torch.Tensor([extra_idx],device=alive_idx.device).unsqueeze(0).expand(B,-1)),dim=1)
            else:
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', keep_rate=(1, ), fuse_token=False, save_num=8, base_keep_rate=1.):
        """
        Args:
        ...
        self.save_token = nn.Parameter(torch.zeros(1, self.save_num, embed_dim))
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.dist_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if distilled else None
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens + self.save_num, embed_dim))
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                keep_rate=keep_rate[i], fuse_token=fuse_token, depth=depth, save_num=save_num,base_keep_rate=self.base_keep_rate)
    ...
    def init_weights(self, mode=''):
        ...
        if mode.startswith('jax'):
            ...
            trunc_normal_(self.save_token, std=.02)
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        save_token = self.save_token.expand(x.shape[0], -1, -1)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        if self.dist_token is None:
            x = torch.cat((save_token, cls_token, x), dim=1)
        else:
            x = torch.cat((save_token, cls_token, self.dist_token.expand(x.shape[0], -1, -1), x), dim=1)
        ...
        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, ST]), left_tokens, idxs
        else:
            return x[:, ST], x[:, ST+1], idxs
...
@register_model
def deit_small_patch16_shrink_base_se_8soi(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, save_num=8,base_keep_rate=base_keep_rate)
...
if __name__ == "__main__":
    ...
    model = deit_small_patch16_shrink_base_se_8soi(base_keep_rate=0.7).cuda()
