...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, num_tokens=1, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1.):
        super().__init__()
    ...
    def forward(self, x, keep_rate=None, tokens=None):
        ...
        x_img = self.forward_global(q_img, k_img, v_img)

        # global aggregation
        x_glb, cls_attn = self.forward_global_aggregation(q_glb, k_img, v_img)
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            left_tokens = math.ceil(keep_rate * (N - NT))
            if tokens is not None:
                left_tokens = tokens
            if left_tokens == N - NT:
                return x, None, None, None, left_tokens
            assert left_tokens >= 1
            cls_attn = cls_attn.mean(dim=1)  # [B, NT, N-NT]
            cls_attn = cls_attn.mean(dim=1)  # [B, N-NT]
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 fuse_token=False, num_tokens=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.num_tokens = num_tokens
        self.attn = Attention(dim, num_heads=num_heads, num_tokens=self.num_tokens, qkv_bias=qkv_bias,
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        if index is not None:
            ...
            if self.fuse_token:
                compl = complement_idx(idx, N - NT)  # [B, N-1-left_tokens]
                ...
                x = torch.cat([x[:, 0:NT], x_others, extra_token], dim=1)
            else:
                x = torch.cat([x[:, 0:NT], x_others], dim=1)
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', keep_rate=(1, ), fuse_token=False, num_tokens=1):
        """
        Args:
        ...
        self.num_tokens = num_tokens + 1 if distilled else num_tokens
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                keep_rate=keep_rate[i], fuse_token=fuse_token, num_tokens=self.num_tokens)
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, :NT].mean(1)), left_tokens, idxs
        else:
            return x[:, :NT].mean(1), x[:, NT], idxs
...
@register_model
def deit_small_patch16_shrink_base_8cls(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, num_tokens=8)
    model_kwargs.update(kwargs)
    model = _create_evit('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def deit_small_patch16_shrink_base_8cls_fuse(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, num_tokens=8, fuse_token=True)
    model_kwargs.update(kwargs)
    model = _create_evit('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

if __name__ == "__main__":
    model = deit_small_patch16_shrink_base_8cls_fuse(num_classes=1000, base_keep_rate=0.7).cuda()
