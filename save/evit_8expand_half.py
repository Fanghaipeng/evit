...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, save_num=0, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1.):
        super().__init__()
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        if index is not None:
            ...
            if self.fuse_token:
                ...
                half = int(non_topk.shape[1]/2)
                extra_token = torch.sum(non_topk * non_topk_attn.unsqueeze(-1), dim=1, keepdim=True)  # [B, 1, C]
                non_topk_half = non_topk[:,:half].clone()
                non_topk_attn_half = non_topk_attn[:,:half].clone()
                extra_token2 = torch.sum(non_topk_half * non_topk_attn_half.unsqueeze(-1), dim=1, keepdim=True)  # [B, 1, C]
                # extra_8tokens = self.expand(extra_token).view(x.shape[0], ST, -1)
                x_save = x[:, 0:ST].clone()
                x[:, 0:ST] = x_save + extra_token2.expand(-1, ST, -1)
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', keep_rate=(1, ), fuse_token=False, save_num=8):
        """
        Args:
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                keep_rate=keep_rate[i], fuse_token=fuse_token, save_num=save_num)
...
@register_model
def deit_small_patch16_shrink_base_8expand_half(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, save_num=8)
    model_kwargs.update(kwargs)
    model = _create_evit('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

# @register_model
# def deit_small_patch16_shrink_base_8save_others_fuse(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
#     keep_rate = [1] * 12
#     for loc in drop_loc:
#         keep_rate[loc] = base_keep_rate
#     model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, save_num=8, fuse_token=True)
#     model_kwargs.update(kwargs)
#     model = _create_evit('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
#     return model

if __name__ == "__main__":
    model = deit_small_patch16_shrink_base_8expand_half(num_classes=1000, base_keep_rate=0.7).cuda()
