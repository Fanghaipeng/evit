    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        if index is not None:
            ...
            if self.fuse_token:
                ...
                x_save = x[:, 0:ST].clone()
                x[:, 0:ST] = x_save + extra_token.expand(-1, ST, -1)
...
@register_model
def deit_small_patch16_shrink_base_8save_expand8(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    model = deit_small_patch16_shrink_base_8save_others(num_classes=1000, base_keep_rate=0.7).cuda()
