...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1.):
        ...
        self.min = torch.tensor([0.]).cuda()
        self.max = torch.tensor([1.]).cuda()
    ...
    def forward(self, x, pos, keep_rate=None, tokens=None):
        ...
        self.min = 0.99 * self.min + 0.01 * mask_min
        self.max = 0.99 * self.max + 0.01 * mask_max
        mask_len = self.max - self.min
        mask = 1 - self.gamma * (mask - self.min)/mask_len
...
@register_model
def deit_small_patch16_shrink_base_l2g3_bmm(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = deit_small_patch16_shrink_base_l2g(base_keep_rate=0.7).cuda()
    img_size = 224
