...
@register_model
def deit_small_patch16_shrink_base_l2nga_lfb_8sto(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    # model = deit_small_patch16_shrink_base_l2nga_lfb_4sto(num_classes=1000, base_keep_rate=0.7).cuda()
    # img_size = 224
    # x = torch.randn(16, 3, img_size, img_size).cuda()
    # model(x)

    model = deit_small_patch16_shrink_base_l2nga_lfb_8sto(num_classes=1000, base_keep_rate=0.5).cuda()
