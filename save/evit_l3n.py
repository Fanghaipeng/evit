...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.zpos = torch.cat((torch.Tensor([1]).cuda(),torch.zeros(patches_h * patches_h).cuda()))
        # ! end
...
@register_model
def deit_small_patch16_shrink_base_l3n(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = deit_small_patch16_shrink_base_l3n(base_keep_rate=0.7).cuda()
    img_size = 224
