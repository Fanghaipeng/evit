...
class Attention(nn.Module):
    ...
    def forward(self, x, pos, keep_rate=None, tokens=None):
        ...
        c1 = torch.unsqueeze(pos_with_cls, dim=2)
        c2 = torch.unsqueeze(pos_with_cls, dim=1)
        mask = (torch.sum((c1 - c2)**2, dim=-1) + 1e-5) ** 0.5
        
        mask_min = torch.min(mask).detach()
        mask_max = torch.max(mask).detach()
        mask_len = mask_max - mask_min
        # mask = 1 - 0.5 * (mask - mask_min)/mask_len
        mask = 1 - self.gamma * (mask - mask_min)/mask_len
        #NOTE： attn 乘 v 时 乘上权重
    ...
    def forward(self, x, pos, keep_rate=None, tokens=None, get_idx=False):
        ...
        if index is not None:
            ...
            if self.fuse_token:
                ...
                # ! new for ps3
                extra_pos = torch.sum(pos_non_topk * non_topk_attn.unsqueeze(-1), dim=1, keepdim=True)  # [B, 1, 3]
                # ! end
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.xpos = nn.Parameter(torch.cat((torch.Tensor([0.5]).cuda(),torch.arange(0, 1, 1/patches_h).cuda().reshape(-1,1).expand(-1,patches_h).reshape(-1))))
        self.ypos = nn.Parameter(torch.cat((torch.Tensor([0.5]).cuda(),torch.arange(0, 1, 1/patches_h).cuda().reshape(1,-1).expand(patches_h,-1).reshape(-1))))
        self.zpos = nn.Parameter(torch.cat((torch.Tensor([1]).cuda(),torch.zeros(patches_h * patches_h).cuda())))
        # ! end
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        posz = self.zpos.view(1,-1,1).expand(x.shape[0], -1, -1)
        pos = torch.cat((posx,posy,posz), dim=2)
        # ! end
...
@register_model
def deit_small_patch16_shrink_base_l3wg(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
...
if __name__ == "__main__":
    ...
    model = deit_small_patch16_shrink_base_l3wg(base_keep_rate=0.7).cuda()
    img_size = 224
