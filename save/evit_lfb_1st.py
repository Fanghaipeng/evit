...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1., base_keep_rate=1., depth=0):
        super().__init__()
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(1, num_heads, 198, 198))  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x, keep_rate=None, tokens=None, alive_idx=None):
        ...
        if self.depth > 3:
            save_tokens = math.ceil(self.base_keep_rate * (N - 2))
            attn[:,:,1,2:save_tokens+2] = 0
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 fuse_token=False, base_keep_rate=1., depth=12):
        super().__init__()
        self.depth = depth
        self.norm1 = norm_layer(dim)
        self.base_keep_rate=base_keep_rate
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias,
                              attn_drop=attn_drop, proj_drop=drop, keep_rate=keep_rate,base_keep_rate=self.base_keep_rate,depth=self.depth)
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, alive_idx=None):
        ...
        if index is not None:
            ...
            alive_idx = torch.cat((alive_idx[:, 0:2],torch.gather(alive_idx, dim=1, index=idx)),dim=1)

            if self.fuse_token:
                compl = complement_idx(idx, N - 2)  # [B, N-1-left_tokens]
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', keep_rate=(1, ), fuse_token=False, base_keep_rate=1.):
        """
        Args:
        ...
        self.num_tokens = 3 if distilled else 2
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                keep_rate=keep_rate[i], fuse_token=fuse_token, base_keep_rate=self.base_keep_rate, depth=i)
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        if self.dist_token is None:
            ...
            return torch.max(self.pre_logits(x[:, 0:2]),1)[0], left_tokens, idxs
        else:
            return torch.max(x[:, 0:2],1)[0], x[:, 2], idxs
...
@register_model
def deit_small_patch16_shrink_base_lfb_1st(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, base_keep_rate=base_keep_rate)
...
if __name__ == "__main__":
    ...
    model = deit_small_patch16_shrink_base_lfb_1st(base_keep_rate=0.7)
    img_size = 224
